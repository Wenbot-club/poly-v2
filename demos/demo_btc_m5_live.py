#!/usr/bin/env python3
"""
BTC M5 paper-live runner — optimised for low-latency execution.

Key differences from demo_btc_m5.py:
  - Token prices stream via Polymarket CLOB WebSocket (not HTTP polling).
    The WS cache is pre-warmed before the entry/hedge window opens.
  - M5Session receives the pre-warmed price dict — no HTTP call on the hot path.
  - Latency timestamps are recorded per order and summarised at end of campaign.

Usage:
  python demos/demo_btc_m5_live.py --windows 6 --output-dir m5_out_live/
...
async def _update_chainlink_loop(
    ...
    """Stream Polymarket Chainlink BTC/USD feed → state."""
...
async def _stream_token_prices_ws(
    ...
    """
    Stream UP/DOWN token prices from Polymarket CLOB WS into price_cache.

    Runs until cancelled.  Reconnects automatically on network errors
    (handled inside PolymarketMarketDataProvider).
    """
...
def _record_latency(record: TradeRecord, tracker: LatencyTracker) -> None:
    """Extract latency records from a settled TradeRecord and add to tracker."""
    ...
    async with aiohttp.ClientSession() as http:
        ...
        try:
            for i in range(window_count):
                ...
                # Resolve prefetched tokens for this window
                ...
                # Shared token price cache — populated by WS stream below
                token_prices: dict = {}

                # Launch WS price stream for this window's tokens (if known)
                ...
                # Stop this window's WS price stream
                ...
                # Prefetch tokens for next window
...
# ---------------------------------------------------------------------------
# Per-window print (minimal, hot-path-safe)
# ---------------------------------------------------------------------------
